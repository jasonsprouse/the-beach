...
export class PKPRedisConfigEncryptor {
  ...
  async init() {
    ...
    console.log('✅ PKP Agent connected to Lit Network');
  }
  ...
  async encryptRedisConfig(config: RedisConfig): Promise<EncryptionResult> {
    console.log('🔐 PKP Agent: Encrypting Redis configuration...');
...
`.trim();
  ...
  async disconnect() {
    ...
    console.log('🔌 PKP Agent disconnected');
  }
}
...
async function main() {
  console.log('🤖 PKP Redis Config Encryptor Agent');
  console.log('====================================\n');
  ...
  console.log('\n📄 The Beach .env configuration:');
  console.log(agent.generateBeachEnvVars(redisConfig));

  console.log('\n📄 Y8 App .env.local configuration:');
