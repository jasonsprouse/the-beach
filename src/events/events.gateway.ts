import {
  ...
  export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
      ...
      if (roomId) {
        ...
        if (roomUsers) {
          ...
          if (roomUsers.size === 0) {
            ...
            this.logger.log(`Notified room ${roomId} about user ${client.id} disconnecting`);
          }
        }
    ...
    handleJoinRoom(
      ...
      if (!room) {
          this.logger.warn(`Client ${client.id} tried to join without specifying a room.`);
          return;
      }
    ...
    handleMove(
      ...
      if (roomId && payload) {
        ...
        // No need to log every move event, usually too noisy
          ...
          this.logger.warn(`Client ${client.id} sent move event but is not in a room.`);
      }
    }
  }